...
exports.getInvoiceByProject = async (req, res) => {
  try {
    const { projectId } = req.params;
    if (!mongoose.Types.ObjectId.isValid(projectId)) {
      return res.status(400).json({ message: 'Invalid project ID' });
    }
    const invoices = await Bill.find({ project_id: projectId })
      .populate('creator_id', 'fullname email')
      .populate('recipient_id', 'fullname email')
      .populate('project_id', 'status');
      console.log("INVOICESS : " +invoices);
    ...
    console.error('Error retrieving invoices by project:', error.message);
    res.status(500).json({ message: 'Error while retrieving invoices', error: error.message });
  }
};