        ...
        return (
          <>
            <li className="profile-user-info-item">
              ...
              <span className="profile-user-info-value">{userData.companyName || 'N/A'}</span>
            </li>
            <li className="profile-user-info-item">
              <span className="profile-user-info-label">Registration Number:</span>
              <span className="profile-user-info-value">{userData.registrationNumber || 'N/A'}</span>
            </li>
            <li className="profile-user-info-item">
              <span className="profile-user-info-label">Industry:</span>
              <span className="profile-user-info-value">{userData.industry || 'N/A'}</span>
            </li>
            <li className="profile-user-info-item">
              <span className="profile-user-info-label">Authorization:</span>
              <span className="profile-user-info-value">
                {userData.autorization ? 'Approved' : 'Pending' || 'N/A'}
              </span>
            </li>
          </>
        );
        ...
        return (
          <>
            <li className="profile-user-info-item">
              ...
              <span className="profile-user-info-value">{userData.certification || 'N/A'}</span>
            </li>
            <li className="profile-user-info-item">
              <span className="profile-user-info-label">Experience Years:</span>
              <span className="profile-user-info-value">{userData.experienceYears || 'N/A'}</span>
            </li>
            <li className="profile-user-info-item">
              <span className="profile-user-info-label">Specialization:</span>
              <span className="profile-user-info-value">{userData.specialization || 'N/A'}</span>
            </li>
          </>
        );
        ...
        return (
          <>
            <li className="profile-user-info-item">
              ...
              <span className="profile-user-info-value">{userData.department || 'N/A'}</span>
            </li>
            <li className="profile-user-info-item">
          ...
          <div className="profile-cards-row">
            ...
            <div className="card">
              <div
                className="image"
                style={{
                  backgroundImage: userData.picture ? `url(${userData.picture})` : undefined,
                  backgroundSize: 'cover',
                  backgroundPosition: 'center',
                }}
              ></div>
              <div className="card-info">
                <span>{`${userData.fullname} ${userData.lastname}`}</span>
                <p>{userData.role}</p>
              </div>
            ...
            {/* Right Column: Wallet Card and Project Card */}
            <div className="profile-right-column">
              {/* Wallet Card */}
              <div className="profile-card" onClick={() => navigate('/Transaction')}>
                <div className="profile-card-inner">
                  <div className="profile-card-front">
                    {/* Updated SVG for wallet icon with fixed circles and no orange gradient */}
                    <svg width="110" height="110" viewBox="0 0 110 110" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ marginBottom: '10px' }}>
                      <circle cx="55" cy="55" r="50" fill="#1769aa" stroke="#fff" strokeWidth="5" />
                      <circle cx="55" cy="55" r="40" fill="none" stroke="#fff" strokeWidth="3" />
                      <circle cx="75" cy="55" r="5" fill="#fff" />
                      <path d="M40 45 H70 V65 H40 Z" fill="#0b4c66" />
                      <path d="M50 65 Q55 70 60 65" stroke="#fff" strokeWidth="2" fill="none" />
                    </svg>
                    <div style={{ fontWeight: 'bold', fontSize: '20px', color: '#fff', marginTop: '10px', textAlign: 'center' }}>
                      Wallet Balance: {walletBalance !== null ? `${walletBalance} TND` : 'Loading...'}
                    </div>
                  </div>
